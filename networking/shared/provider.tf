...
terraform {
  cloud {
    ...
    workspaces {
      name = "az-meltaier-landing-zone-networking-shared"
    }
  }
}
...
provider "azurerm" {
  ...
  subscription_id = data.terraform_remote_state.query_subscriptions.outputs.subscriptionConnectivity.subscription_id
